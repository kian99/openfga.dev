...
<details>
<summary>
  Note that <ProductName format={ProductNameFormat.LongForm}/> does not evaluate the related object (the word after `from`, also called the tupleset), it will only consider directly related objects, and will not perform any evaluation (e.g. any rewrites on that relation, or `*`, etc..).
</summary>
  In the case below:
...
/>
  ...
  And then run the following check, it will return `false`, because the `ancestor` relation contains a rewrite which will not be evaluated.
